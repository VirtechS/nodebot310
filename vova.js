...
cm.use(async (message, next) => {
	...
	if(/\[club173290101\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club173290101\|(.*)\]/ig, '').trim();
...
cm.hear(/^(?:стоп сапёр|стоп сапер)/i, message => {
	const a = acc.users[user_id(message.user)];	
	if(!saper[message.user] || !saper[message.user].status) return message.send('Дэбил, сапера запусти, даунич')
...
/*cm.hear(/^(?:токены)/i, message => { 
	if(message.user != 155207640) return
 ...
 return message.send(`
	...
	return message.send(`@id${user.id}(${user.prefix}), новый статус профиля: Закрытый`); // Исполнительный текст
     	   ...
     	   cm.hear(/^(?:ID|Мой ид|мой ID)/i,  (message) => { // Сама команда
     	   	let user = acc.users[user_id(message.user)]; 
     	   	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	return message.send(`@id${user.id}(${user.prefix}), ID Вашего аккаунта в @mimk310 (Бот Вова): ${user_id(message.user)}\nID Вашего профиля ВКонтакте: @id${user.id}(${user.id})`); // Исполнительный текст
});
...
cm.hear(/^(?:FD)\s?([0-9]+)?/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(message.user != 155207640) return;
	    if(!message.$match[1]) return message.send(`Что-то пошло не так 😱\n Подсказка: Пример команды: FD [ID] \n -- FD это - Выдача полного доступа к системе @mimk310 (Бот Вова) `) // Подсказка для команды
...
cm.hear(/^(?:DFD)\s?([0-9]+)?/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(message.user != 155207640) return;
	    if(!message.$match[1]) return message.send(`Что-то пошло не так 😱\n Подсказка: Пример команды: DFD [ID] \n -- DFD это - Снятие полного доступа с игрока в системе @mimk310 (Бот Вова) `) // Подсказка для команды
...
cm.hear(/^(?:untiban)\s?([0-9]+)?/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(message.user != 155207640) return;
	    if(!message.$match[1]) return message.send(`Что-то пошло не так 😱\n Подсказка: Пример команды: UNTIBAN [ID] \n -- UNTIBAN это - Выдача Анти блокировки игроку в боте: @mimk310 (Бот Вова) `) // Подсказка для команды
...
cm.hear(/^(?:duntiban)\s?([0-9]+)?/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(message.user != 155207640) return;
	    if(!message.$match[1]) return message.send(`Что-то пошло не так 😱\n Подсказка: Пример команды: DUNTIBAN [ID] \n -- DUNTIBAN это - Снятие Анти блокировки с и грока в боте: @mimk310 (Бот Вова) `) // Подсказка для команды
	...
	if(!message.$match[1]) return message.send(`Уважаемый @id${user.id} (Основатель), произошла ошибка!\n -- Для того что бы включить или выключить бонус введите: bonus [баланс/опыт] [вкл/выкл]`)
		...
		if(message.$match[1] == 'баланс'){
			if(message.$match[2] == 'вкл'){ config.bonus_balance = true; 
				message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы включили х2 баланс для всех игр/бонусов в @mimk310 (Бот Вова)`); 
			...
			if(message.$match[2] == 'выкл'){ config.bonus_balance = false; 
				message.send(`@id${user.id}(${user.prefix}), Вы выключили х2 баланс для всех игр/бонусов @mimk310 (Бот Вова)`); 
				return message.send({sticker_id:4658});
			}
		}
		if(message.$match[1] == 'опыт'){ 
			if(message.$match[2] == 'вкл'){ config.bonus_exs = true; 
				message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы включили х2 опыт для всех игр/бонусов в @mimk310 (Бот Вова)`); 
				message.send({sticker_id:4649})
				return message.send({attachment:`audio449532928_456239406`});
			} 
			if(message.$match[2] == 'выкл'){ config.bonus_exs = false; 
				message.send(`@id${user.id}(${user.prefix}), Вы выключили х2 опыт для всех игр/бонусов @mimk310 (Бот Вова)`); 
				return message.send({sticker_id:4658});
			}
...
vk.updates.hear(/^(?:гиф|gif)\s(.*)$/i, async (message, bot) => {
	...
	.then(response => { 
		...
		return message.send(`${rnick}, по вашему запросу [${message.$match[1]}], я нашел следующие GIF Материалы:`, {attachment: items}) 
	}) 
});
...
vk.updates.hear(/^(?:Vipe|вайп)\s?([^]+)?/i,  message => {
	let user = acc.users[user_id(message.user)];
	if(message.user != 155207640) return; 
	if(!message.$match[1]) return message.send(`Уважаемый @id155207640 (Основатель), Для того что бы произвести вайп введите: Вайп [рейтинга/баланса/администрации/uptime]`);  
	...
	if(message.$match[1] == 'рейтинга'){ 
		...
		message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы Онулировали всем игрокам рейтинг в @mimk310 (Бот Вова)`); 
		message.send({sticker_id:4649})
		return message.send({attachment:`audio449532928_456239406`});
	}
	if(message.$match[1] == 'баланса'){ 
		...
		message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы Онулировали всем игрокам баланс в @mimk310 (Бот Вова)`); 
		message.send({sticker_id:4649})
		return message.send({attachment:`audio449532928_456239406`});
	}
	if(message.$match[1] == 'администрации'){ 
		...
		message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы сняли всю Администрацию @mimk310 (Бот Вова)`); 
		message.send({sticker_id:4649})
		return message.send({attachment:`audio449532928_456239406`});
	}
	if(message.$match[1] == 'uptime'){ 
		...
		message.send(`@id${user.id}(${user.prefix}), Успех!\n Вы Онулировали счетчик UpTime всем игрока @mimk310 (Бот Вова)`); 
		message.send({sticker_id:4649})
		return message.send({attachment:`audio449532928_456239406`});
	}
}); 
	...
	return message.send(`@id${user.id}(${user.prefix}), Вы купили телефон  (${names[i]}) за ${count[i]}$ !\nЧтобы зайти в меню, напишите "phone"\n Советуем вам купить сим-карту "buysim"`)
      	      ...
      	      vk.updates.hear(/^(?:правила)/i, (message) => {
      	      	...
      	      	return message.send(`
      	      		...
      	      		🔸 1. Сдавать практики по тазику
      	      		🔸 2. Сдавать практики по ВГ 
      	      		🔸 3. Сдавать практики по Ануру.  
      	      		🔸 4. Не страдать хуйней
					🔸 5. Курить гашик
					🔸 6. Быть пиздатым
      	      ...
      	      vk.updates.hear(/^(?:Регистрация|ригистрация|ригестрация|рег|риг|ригиструция|рагестрация|регестрация)/i, (message) => { 
      	      	let user = acc.users[user_id(message.user)];
      	      	if(user.act == true) return message.send(`@id${user.id}(${user.prefix}), Вы уже зарегистрированны в системе @mimk310 (Бот Вова)!\n -- Ваши команды: "Помощь"`);
      	      	user.act = true
      	      ...
      	      vk.updates.hear(/^(?:arules)/i,  (message) => { 
      	      	...
      	      	return message.send(`
      	      		...
      	      		🔸 1. Следить ебать за порядком большего и не нужно, если что бить в ебало.
      	     ...
      	     /* vk.updates.hear(/^(?:раздача)$/i, async (message) => { 
      	      	...
      	      	let smile = ['🙂','😯','☺','🤑','😉'].random();
      	      	let nsmile = ['🎊','⭐','🔊','🔥','🎮','🌈','💡','💳','🏆','🎉','💸'].random();
      	      	user.api.wall.post({owner_id: -173290101, message: `🏆 Доброго времени суток! Сделайте репост этой записи и получите: ${spaces(bvin)}$ на свой игровой баланс и ${rvin}₽ на свой донат счёт.
      	      		-- Начало акции в ${time()}. Длительность акции 3 часа.`, attachments: 'photo-179018727_456239024'}).then((response) => { 
      	      			user.api.wall.openComments({owner_id: -173290101, post_id: response.post_id });
      	      			user.api.wall.createComment({owner_id: -173290101, post_id: response.post_id, from_group: 173290101, message: '' });
      	      			user.api.wall.createComment({owner_id: -173290101, post_id: response.post_id, from_group: 173290101, message: `*Вы должны быть зарегистрированы в боте, а ваш профиль ВКонтакте открыт.\n-- Валюта будет выдана по покончанию акции, об этом вы будите оповещены в личных сообщениях бота.`});
      	      			user.api.wall.closeComments({owner_id: -173290101, post_id: response.post_id});
      	      			message.send(`Вы успешно запустили раздачу.\n Ссылка на раздачу: [vk.com/wall-132550063_${response.post_id}].`)

      	      			setTimeout(() => {
      	      				user.api.wall.getReposts({owner_id: -173290101, post_id: response.post_id, count: 1000}).then((res) => { 
      	      					res.items.map(x=> { 
      	      						...
      	      						vk.api.messages.send({user_id: 155207640, message: `[Авто-Раздача]: Игроку "(@id${user.id} (${user.prefix})" выдано: ${spaces(bvin)}$ и ${rvin}₽ на баланс.\n\nЕго баланс составляет: ${spaces(user.balance)}$ и ${spaces(user.rub)}₽`}) 
      	      					}); 
      	      				});

      	      				user.api.wall.openComments({owner_id: -173290101, post_id: response.post_id }); 
      	      				user.api.wall.createComment({owner_id: -173290101, post_id: response.post_id, from_group: 173290101, message: '' }); 
      	      				user.api.wall.createComment({owner_id: -173290101, post_id: response.post_id, from_group: 173290101, message: '*Раздача окончена. Всем участникам Акции призы были успешно начисленны.'}); 
      	      				user.api.wall.closeComments({owner_id: -173290101, post_id: response.post_id});
      	      				giving = false; 
      	      			}, 18000000);
      	      		}); 
 ...
 vk.updates.hear(/^(?:помощь|начать|хелп|help|команды|📚 Помощь)$/i,  (message) => { 
 	...
 	message.send(`
		...
		📚 Учеба - [➕] Команды для учебы.
 		...
 		🙄Юля [текст] - начать общение с ботом голосовыми сообщениями (В РАЗРАБОТКЕ)
...
vk.updates.hear(/^(?:!kick|!кик|!кикнуть)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => {
	...
	if(message.$match[4]) { 
		...
		vk.api.call("utils.resolveScreenName", { 
			...
			if(res.object_id == 155207640) return message.reply('⛔  Произошла критическая ошибка системы\n -- Подсказка: @id155207640 (Разработчика) невозможно кикать из бесед!');  
		...
		if(message.$match[3] == 155207640) return message.reply('⛔ Произошла критическая ошибка системы\n -- Подсказка: @id155207640 (Разработчика) невозможно кикнуть из бесед!'); 
...
vk.updates.hear(/^(?:поиск|search)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => {
	...
	if(message.$match[3]){
		...
		return message.send(`
			...
			ID Игрока в @mimk310 (Бот Вова): ${id}
...
vk.updates.hear(/^(?:рассылка)\s?([^]+)?/i,  message => { 
	...
	if(message.user != 155207640) return message.send(`⚠ Доступ закрыт ⚠`);
	for(i in acc.users){
		vk.api.call('messages.send', {
...
// Рассылки для беседы
        vk.updates.hear(/^(?:ch)\s?([^]+)?/i,  message => {  // сообщение
        	...
        	if(message.user != 155207640) return message.send(`⚠ Доступ закрыт ⚠`);
        	vk.api.call('messages.send', {
			chat_id: 1, //Беседа Бот Вова
			message: `${message.$match[1]}`,
						random_id: 0 // Сообщение
					});
        	return message.send(`Сообщение успешно отправлено в Официальную беседу @mimk310 (Бот Вова)`);
        });
         ...
         vk.updates.hear(/^(?:cp)\s?([^]+)?\s([^]+)?/i, (message) => { // сообщение и пост
			...
			attachment: `${message.$match[2]}`,
         	...
         	return message.send(`Сообщение c вложением успешно отправлено в Официальную беседу @mimk310 (Бот Вова)`);
         });

                vk.updates.hear(/^(?:cmuz)\s?([^]+)?\s([^]+)?/i, (message) => { // Музыка
                	if(message.user != 155207640) return message.send(`⚠ Доступ закрыт ⚠`);
                	vk.api.call('messages.send', {
			chat_id: 1, //Беседа Вова
			message: `${message.$match[1]}\n`, // Сообщение
			attachment: `${message.$match[2]}`,
						random_id: 0 // Вложение
					});
                	return message.send(`Музыка успешно отправлена в Официальную беседу @mimk310 (Бот Вова)`);
                });
         ...
         vk.updates.hear(/^(?:ct)\s?([^]+)?/i,  message => { // стикер
			...
			sticker_id: `${message.$match[1]}`,
         	...
         	return message.send(`Стикер успешно отправлен в Официальную беседу @mimk310 (Бот Вова)`);
         });
	...
	if(user.block_give == true) return message.send(`@id${user.id}(${user.prefix}), вам был заблокирован доступ к передачи средств`)
		...
		return message.send(`@id${user.id}(${user.prefix}),  вы передали игроку ${acc.users[message.$match[1]].prefix} ${message.$match[2]}$. 😉`);
              	        ...
              	        vk.updates.hear(/^(?:giv)\s?([0-9]+)?\s?([^\s  ].*)?/i,  message => {
              	        	...
              	        	adm_log(is)
              	        	return message.send(`💰 @id${user.id}(${user.prefix}), Успех! 😎\n [Система @mimk310 (Бот Вова)]: Зачисляю ироку @id${acc.users[message.$match[1]].id}(${acc.users[message.$match[1]].prefix}) - ${spaces(message.$match[2])}$\n\n Баланс игрока: ${acc.users[message.$match[1]].balance}$`);
...
vk.updates.hear(/^(?:sms|смс)\s?([0-9]+)?\s([^]+)?/i, (message) => { 
	...
	}).then((res) => {}).catch((error) => {console.log('[Система]: Женя! Я обнаружила ошибку в системе: SMS-Сообщение'); });		
	var is = [user_id(message.user), message.text] 
...
vk.updates.hear(/^(?:setnick)\s?([0-9]+)?\s([^]+)?/i, (message) => { 
	...
	vk.updates.hear(/^(?:delladmin)\s?([0-9]+)?\s([^]+)?/i, (message) => {  
		...
		if(message.$match[1] == 1) return message.send(`Уважаемый @id${user.id}(Администратор), к сожалению @id155207640 (Разработчика) невозможно снять!`);
...
vk.updates.hear(/^(?:CMD)$/i, (message) => { 
	...
	if(user.full == false) return;
	{
		return message.send(`Команды @id155207640 (Разработчика)
			👑 Основные:
...
/*vk.updates.hear(/^(?:Стикер)\s?([0-9]+)?/i,  message => {
	...
	message.send({
		sticker_id: stick}).catch((error) => {return message.send(`😢 ${rnick}, к сожалению, мой @id155207640 (Владелец) не купил мне ещё пак в котором будет стикер №${message.$match[1]}`)});
});*/
...
vk.updates.hear(/^(?:permban)\s?([0-9]+)?\s([^]+)?/i, (message) => {  
	...
	if(acc.users[message.$match[1]].ban == true) return message.send(`@id${user.id}(${user.prefix}), Что-то пошло не так 😱\n -- Аккаунт игрока ${acc.users[message.$match[1]].prefix} уже имеет перманентную блокировку\n Для разблокировки, используйте: UnPermBan ${message.$match[1]}.`);
	if(!acc.users[message.$match[1]]) return message.send(`@id${user.id}(${user.prefix}), Что-то пошло не так 😱\n Пользователь не обнаружен в Базе данных.\n Подсказка: Возможно вы не правельно ввели ID Пользователя. Пожалуйста, проверьте правильность введенных данных.`);
	if(message.$match[1] == 2) return message.send(`🔸 Уважаемый @id${user.id}(Администратор), к сожалению @id155207640 (Разработчику) невозможно выдать временную блокировку.`);
...
vk.updates.hear(/^(?:warn)\s?([0-9]+)?\s([^]+)?/i, (message) => { 
	...
	if(acc.users[message.$match[1]].ban == true) return message.send(`@id${user.id}(${user.prefix}), Что-то пошло не так 😱\n -- Аккаунт игрока ${acc.users[message.$match[1]].prefix} Уже заблокирован навсегда.`);
	if(!acc.users[message.$match[1]]) return message.send(`🔸 @id${user.id}(${user.prefix}), Что-то пошло не так 😱\n Пользователь не обнаружен в Базе данных.\n Подсказка: Возможно вы не правельно ввели ID Пользователя. Пожалуйста, проверьте правильность введенных данных.`);
	if(message.$match[1] == 2) return message.send(`Уважаемый @id${user.id}(Администратор), к сожалению @id155207640 (Разработчику) невозможно выдать Предупреждение`);
...
vk.updates.hear(/^(?:AVIG)\s?([0-9]+)?\s([^]+)?/i, (message) => { 
	...
	if(!Number(message.$match[1])) return message.send(`🔸 @id${user.id}(${user.prefix}), Что-то пошло не так 😱\n Подсказка: ID Пользователя должен быть цифрового вида.`);
	if(!acc.users[message.$match[1]]) return message.send(`🔸 @id${user.id}(${user.prefix}), Что-то пошло не так 😱\n Пользователь не обнаружен в Базе данных.\n Подсказка: Возможно вы не правельно ввели ID Пользователя. Пожалуйста, проверьте правильность введенных данных.`);
	if(message.$match[1] == 2) return message.send(`Уважаемый @id${user.id}(Основатель), к сожалению @id155207640 (Разработчику) невозможно выдать выговор`);
...
vk.updates.hear(/^(?:unban)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @mimk310 (Бот Вова)]: С вашего аккаунта досрочно была снята временная блокировка. \n -- Больше не нарушайте :)`,			random_id: 0
	});
...
vk.updates.hear(/^(?:unpermban)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @mimk310 (Бот Вова)]: С вашего аккаунта была снята перманентная блокировка.\n -- Больше не нарушай :)`,			random_id: 0
	});
	var is = [user_id(message.user), message.text] 
	adm_log(is)
...
vk.updates.hear(/^(?:unwarn)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @mimk310 (Бот Вова)]: С вашего аккаунта были сняты все предупреждения.\n -- Больше не нарушай :)`,			random_id: 0
	});
	var is = [user_id(message.user), message.text] 
	adm_log(is)
...
vk.updates.hear(/^(?:unavig)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @mimk310 (Бот Вова)]: С вашего аккаунта были сняты все Административные выговора.\n -- Больше не нарушай :)`,			random_id: 0
	});
	var is = [user_id(message.user), message.text] 
	adm_log(is)
...
vk.updates.hear(/^(?:ungban)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @mimk310 (Бот Вова)]: Вам была разблокированна возможность выдачи валюты. [giv]\n -- Больше не нарушайте :)`,			random_id: 0
	});
	var is = [user_id(message.user), message.text] 
	adm_log(is)
...
vk.updates.hear(/^(?:unrban)\s?([0-9]+)?/i, (message) => { 
	...
	vk.api.call('messages.send', {
		peer_id: acc.users[message.$match[1]].id,
		message: `[Система @mimk310 (Бот Вова)]: Вам была разблокированна возможность писать жалобы в репорт.\n -- Больше не нарушайте :)`,			random_id: 0
	});
	var is = [user_id(message.user), message.text] 
	adm_log(is)
...
vk.updates.hear(/^(?:sex|секс|заняться сексом)\s?([0-9]+)?/i, (message) => { 
	...
	if(message.$match[1] == 1) return message.send(`Уважаемый @id${user.id}(пользователь), к сожалению эту команду нельзя использовать на @id155207640 (Разработчике)!`);
	var is = [user_id(message.user), message.text] 
	user.block_por = true;
	setTimeout(() => {
...
vk.updates.hear(/^(?:kiss|поцеловать)\s?([0-9]+)?/i, (message) => { 
	...
	if(user.level < 1) return message.send(`🔸 ➾ Вы не VIP-Пользователь`);
	if(!acc.users[message.$match[1]]) return message.send(`❎ ➾ Игрок не найден!`);
	if(message.$match[1] == 2) return message.send(`Уважаемый @id${user.id}(пользователь), к сожалению эту команду нельзя использовать на @id155207640 (Разработчике)!`);
	var is = [user_id(message.user), message.text] 
	user.block_porn = true;
	setTimeout(() => {
		user.block_porn = false;
...
vk.updates.hear(/^(?:о боте)$/i, (message) => {
	...
	return message.send(`
		...
		📝 Проект: @mimk310 (Бот Вова)


		💻 Система
		💻 » Зарегистрировано Аккаунтов: ${acc.number}
		💻 » UpTime @mimk310 (Бот Вова): Дн: ${uptime.days} || Ч: ${uptime.hours} || Мин: ${uptime.min} || Сек: ${uptime.sec}
		💻 » Cообщений: ${acc.msg}
		💻 » Версия системы: 1.2
		`);
});
...
vk.updates.hear(/^(?:игропрофиль)/i, (message) => { 
	...
	return message.send(`
		...
		💳 Донат счёт: ${user.rub} Тазик Коинов
		🔑 Ключей от кейсов: ${user.keys}
		...
		if(!message.$match[1]){
			...
			return message.send(`
				...
				`+(id.verify == false ? `⛔ Верификация: не подтверждён\n` : `✓ Подтверждёный Профиль.\n
					Эта отметка означает, что профиль ${id.prefix} подтвержден администрацией @mimk310 (Бот Вова).\n\n`)+
				...
				💳 Донат счёт: ${id.rub} Тазик Коинов
				` +(id.youtube == false ? `💻 YouTube Канал: Отсутствует\n` : `💻 YouTube Канал: ${id.youtube}\n`)+`
				` +
				(id.brak == false ? `👫 Партнер: Не женат\n` : `👫 Партнер:   ${acc.users[id.brak].prefix}\n`)+
				`
...
vk.updates.hear(/^(?:погода|weather)/i, async (message, bot) => { 
	...
	.then((res) => { 
		let Utils = { 
			...
			function TempTo () { 
				if(res.main.temp < -15) return 'очень холодно' 
...
vk.updates.hear(/^(?:setpromo)\s?([0-9]+)?/i, message => {
 ...
 vk.updates.hear(/^(?:settimevip)?\s([0-9]+)?\s?([0-9]+)?/i, (message) => { 
 	let user = acc.users[user_id(message.user)];
 	if(user.full == false) return;
 	let id = user_id(message.user);
 ...
 vk.updates.hear(/^(?:settimemoder)?\s([0-9]+)?\s?([0-9]+)?/i, (message) => { 
 	let user = acc.users[user_id(message.user)];
 	if(user.full == false) return;
 	let id = user_id(message.user);
 ...
 vk.updates.hear(/^(?:settimeadm)?\s([0-9]+)?\s?([0-9]+)?/i, (message) => { 
 	let user = acc.users[user_id(message.user)];
 	let id = user_id(message.user);
 	if(user.full == false) return;
 ...
 cm.hear(/^(?:setfull)\s?([0-9]+)?\s?([0-9]+)?/i,  (message) => { 	 
 	if (message.user != 155207640) {
 ...
 vk.updates.hear(/^(?:питомцы)\s?([0-9]+)?/i, message => {
 	...
 	let i = message.$match[1];
 	let user = acc.users[user_id(message.user)];  
 	let names = [0,'Улитка','Кит','Овца','Курица','Коала','Оса','Свинья','Слон','Мартышка','Пингвин','Тигр','Волк','Заяц','Корова','Лев','Юлька','Вавилина','Тазик']
 	if(i < 0 || i > 18) return;
 ...
 vk.updates.hear(/^(?:дома)\s?([0-9]+)?/i, message => {
 	if(!message.$match[1]){  
 		return message.send(`
 			...
 			🔸 8. Дача (1.500.000$) 
			🔸 9. Дом Вовы (1.800.000$) 
 			🔸 10. Дом на пляже (2.000.000$) 
 			🔸 11. Большой Коттедж (5.000.000$) 
 			🔸 12. Особняк (11 ❤) 
 			🔹 13. Дом на Рублёвке (150 ❤) 
 	...
 	let i = message.$match[1];
 	let user = acc.users[user_id(message.user)]; 
 	let count = [0, 20000,50000,150000,300000,500000,700000,1000000,1500000,1800000,2000000,5000000,100,150,300];
 	let names = [0, 'Коробка','Подвал','Палатка','Домик на дереве','Полуразрушенный дом','Дом в лесу','Дом в Европе','Дача','Дом Вовы','Дом На Пляже','Большой Коттедж','Особняк','Дом на Рублёвке','Личный небоскрёб']
 	if(i < 0 || i > 14) return;
...
vk.updates.hear(/^(?:лотерея)/i, (message) => { 
	...
	if(rez == false){
		...
		let count = [3000,5000,10000,15000,20000,40000,80000,160000,20000].random();
		user.balance += count;
...
//------------------------------------------------------------------------------------\\
 ...
 vk.updates.hear(/^(?:вова,|вовчик|трифон|трифоныч,)/i, message => { 
 	let user = acc.users[user_id(message.user)];
 	let rnick = (user.anick ? `@id${user.id}(${user.prefix})` : `${user.prefix}`)
 ...
 /*vk.updates.hear(/^(?:nakruntka)/i, message => {
 	...
 	message.send(`Накрутка комментариев успешно зпущена\n -- Пост на который идёт накрутка  | https://vk.com/wall155207640_1428 |`)
 });/*/
...
/*vk.updates.hear(/^лю тебя аня/i, async (context) => {
	...
	await Promise.all([context.send('@id155207640(и я) тебя @id491223810(Любимый)😍❤❤ @id155207640(тьмок)💋'), context.sendAudioMessage(link)]);
});/*/
...
/*cm.hear(/^мурк/i, async (context) => { 
	...
	const link = gs[Math.floor(Math.random() * gs.length)]; 
	await Promise.all([context.send('@id155207640(мурррчу) только для @id491223810(тебя❤)'), context.sendAudioMessage(link)]); 
});/*/
...
/*vk.updates.hear(/^(?:skin 256)/i,  (message) => { // Сама команда
	...
	let rnick = (user.anick ? `@id${user.id}(${user.prefix})` : `${user.prefix}`)
	if(message.user != 155207640) return;
	message.send(`@id155207640 (Женя) вы сменили свой скин на ID: 256`,{attachment:`photo-179018727_456239018`});
...
/*vk.updates.hear(/^(?:!vkNick)/i,  (message) => {
	...
	vk.api.call('users.get', {user_ids: message.user}).then(res => {let user = res[0]; 
		acc.users[user_id(message.user)].prefix = `${user.first_name} ${user.last_name}`;});
	return message.send(`[Чат-Менеджер @mimk310 (Бот Вова)]: ${rnick}, вы успешно установили себе ник по данным вашей @id${user.id} (странице ВКонтакте)`);
});/*/
...
/*vk.updates.hear(/^(?:!tkick|!ткик)\s?([0-9]+)?\s([0-9]+)?\s([^]+)?/i, (message) => { 	
	...
	if(id == 155207640) return message.reply('err Unknown del users development ⛔');
	vk.api.call("messages.send", {
		chat_id: ch,
...
cm.hear(/^(?:time)/i, message => {
	return message.send(`Точное время у @id155207640(Жени): \n----------------------
		${time(1)} (МСК)
		`);
});

/*cm.hear(/^(?:юля,|юличка,|юленька,)/i, message => {
	const googleTTS = require('google-tts-api');
...
/*cm.hear(/^(?:группа)/i, (message) => {
	...
	return message.send(`[bot.anya|Ссылка на нашу группу: ]`, {
		attachment: 'photo-173290101_456239099'
	});
});/*/
...
vk.updates.hear(/^(?:makehouse)\s?([0-9]+)?\s([^]+)?/i, (message) => {		
	let user = acc.users[user_id(message.user)];
	if(!message.$match[1] || !acc.users[message.$match[1]] || !message.$match[2]) return message.send(` ➾ Пример команды: makehouse [ID] [название]`);
	if(!Number(message.$match[1])) return message.send(`🔸 ➾ Число должно быть цифрового вида.`);
...
vk.updates.hear(/^(?:инфоАккаунт)$/i, (message) => {
	return message.send(`
		🌀 Для того, чтобы подтвердить свой аккаунт и получить статус "подтвержден" вам нужно быть пиздатым. Это очень Важно !
		...
		🛑 Все заявки на подтверждения рассматриваются Женей.
...
vk.updates.hear(/^(?:развлечения)$/i, (message) => {
	...
	message.send(`
		...
		📈 Инфа [Слово] (NEW)
		💕 Совместимость [Имя парня/девушки] (NEW)
		💢 Погода [Город] - Покажет точную погоду по запросу
		💞 GIF [Запрос] - покажет вам GIF материал по запросу (NEW)
		☠ Памятник [пересланное сообщение] - Сделает памятник (NEW)
...
vk.updates.hear(/^(?:полезное)$/i, (message) => {
	...
	message.send(`
		...
		🗣 Скажи [текст] - озвучит ваше сообщение. (NEW)
		🏆 Топ - Топ по рейтингу
		💳 Баланс
...
//УЧЕБА ВСЯ//
vk.updates.hear(/^(?:учеба)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
		📚 Учеба:
		...
		📘Замена число.месяц.год [Пример: Замена 23.01.2020]

		📖Методичка Фамилия [Пример: Методичка Вавилина]
		`)
});

vk.updates.hear(/^(?:Бот понедельник)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
		📅Стандартное расписание на понедельник📅
...
vk.updates.hear(/^(?:Бот вторник)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
...
vk.updates.hear(/^(?:Бот среда)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
		📅Стандартное расписание на среду📅 

		1⃣ МДК 01.02 (SQL+C#) Анурьева Ю.В 
		2⃣ МДК 02.01 (1С) Вавилина В.Г 
		3⃣ МДК 02.02 (Project) Вавилина В.Г
		`)
});

vk.updates.hear(/^(?:Бот четверг)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
		...
		2⃣ МДК 02.01 (1С) Вавилина В.Г 
		3⃣ МДК 01.01 (SQL+C#) Анурьева Ю.В
		`)
});

vk.updates.hear(/^(?:Бот пятница)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
		...
		3⃣ МДК 01.02 (SQL+C#) Анурьева Ю.В
		`)
});

vk.updates.hear(/^(?:Бот суббота)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
		📅Стандартное расписание на субботу📅 

		1⃣ МДК 01.01 (SQL+C#) Анурьева Ю.В
		2⃣ МДК 01.01 (SQL+C#) Анурьева Ю.В 
		3⃣ Компьютерные сети Тазетдинова А.Г
		`)
});

vk.updates.hear(/^(?:Бот расписание)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
		📅Стандартное расписание на понедельник📅
...
vk.updates.hear(/^(?:Замены 23.01.2020)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
		на 1,2 пару идет вторая подгруппа на ВГ а первая группа спит
	на 3-4 пару идет 1 группа на ВГ а 2 группа к Ануру
		`)
});

vk.updates.hear(/^(?:Замены 30.01.2020)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
		2 Группа приходит к 3 паре, 1 группа бибу сосет на все 4 идет
		`)
});
...
vk.updates.hear(/^(?:Замены 31.01.2020)$/i, (message) => {
	let user = acc.users[user_id(message.user)];
	if(user.act == false) return message.send(`@id${user.id}(${user.prefix}), Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`);
	message.send(`
		1 группа идет только на вторую пару, 2 группа на 1,2,3,4
		`)
});
...
/*vk.updates.hear(/^(?:!setname)\s([^]+)?/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.full == false) return message.send(`[Чат-Менеджер Бот Вова]: ${rnick}, Ваш Аккаунт не активирован!\n — Для активации пиши 'Регистрация'. \n— После чего следуйте дальнейшим инструкциям.`); 
	if(message.$from.type == 'user') return message.send(`[Чат-Менеджер Бот Вова]: ⛔ Произошла критическая ошибка системы\n — Подсказка:команда работает только в беседах!`);
	if(user.full == false) return; 
	...
	message.send(`[Чат-Менеджер Бот Вова]: Название беседы было успешно изменено на: ${message.$match[1]}`, {attachment: `audio449532928_456239452`}); 
...
vk.updates.hear(/^(?:pushupdate)$/i, (message) => { 
	let user = acc.users[user_id(message.user)]; 
	if(user.full == false) return;
	{
		return message.send(`
			👑 РЕЗИЛ 31.01.2020 Versio 1.2

			Системы в патче:
			0 Система Инфа
			1 Система Совместимость
			2 Система GIF
			3 Система памятник
			4 Система скажи API
			
			Бот в стадии early access, при обнаружении ошибок просьба написать в репорт о баге "репорт [TEXT]"
		  				`);
		  		}
		  	});	